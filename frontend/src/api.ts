...
export const api = {
  ...
  async getScheduledEmails(page: number = 1, limit: number = 50): Promise<{ emails: EmailJob[]; total: number }> {
    const res = await client.get(`/emails/scheduled?page=${page}&limit=${limit}`);
    return res.data;
  },

  async getSentEmails(page: number = 1, limit: number = 50): Promise<{ emails: EmailJob[]; total: number }> {
    const res = await client.get(`/emails/sent?page=${page}&limit=${limit}`);
    return res.data;
  },

  async searchEmails(query: string, status?: string): Promise<{ emails: EmailJob[]; total: number; source: string }> {
    const res = await client.get(`/emails/search?q=${encodeURIComponent(query)}${status ? `&status=${status}` : ''}`);
    return res.data;
  },
  ...
  async saveSlackWebhook(webhookUrl: string, channel?: string): Promise<{ success: boolean }> {
    const res = await client.post('/slack/webhook', { webhookUrl, channel });
    return res.data;
  },
